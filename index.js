...
function writeToFile(fileName, data) {
  ...
  if (!fs.existsSync(outputDirPath)) {
    fs.mkdir("./output", (error) => {
      if (error) console.log("Error while creating directory ./output", error);
    });
  }
...
function generateTOC() {
  ...
  readmeContentsArray.splice(
    3,
    0,
    renderSection(constants.SECTION_HEADINGS.TOC, generatedTOC, true)
  );
}
...
function renderSection(sectionHeading, sectionContents, noAnchorTagInHeading) {
  ...
  return (
    renderLineForReadMe(
      constants.TEXT_STYLES.SECTION_HEADING +
...
function renderDescription(description) {
  return renderSection(constants.SECTION_HEADINGS.DESC, description);
}
function renderInstallation(installationInstructions) {
  return renderSection(
    constants.SECTION_HEADINGS.INSTALLATION,
    installationInstructions
  );
}
function renderUsage(usageInstructions) {
  return renderSection(constants.SECTION_HEADINGS.USAGE, usageInstructions);
}
function renderTestInstructions(testInstructions) {
  return renderSection(
    constants.SECTION_HEADINGS.TEST_INSTRUCTIONS,
    testInstructions
  );
}
function renderQuestions(githubProfileName, emailAddress) {
  return renderSection(
    constants.SECTION_HEADINGS.QUESTIONS,
    renderGitHubLink(githubProfileName) + renderEmailLink(emailAddress)
  );
}
function renderCredits(credits) {
  return renderSection(constants.SECTION_HEADINGS.CREDITS, credits);
}
function renderLicense(answers) {
  return renderSection(
    constants.SECTION_HEADINGS.LICENSE,
    gm.renderLicenseSectionBody(answers)
  );
}
